...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!order.qb_po_id) throw new Error("Purchase Order has not been pushed to QuickBooks yet. Push the PO first.");
    ...
    if (!mfg?.qb_vendor_id) throw new Error(`Manufacturer "${mfg?.name || "unknown"}" has no QuickBooks vendor ID. Link the vendor in Settings before creating bills.`);
    ...
    // Read existing PO from QB to verify it's there
    steps.push(`reading PO ${order.qb_po_id} from QB`);
    const poResp = await fetch(`${baseUrl}/v3/company/${realm_id}/purchaseorder/${order.qb_po_id}?minorversion=73`, { headers: { Authorization: `Bearer ${access_token}`, Accept: "application/json" } });
    if (!poResp.ok) throw new Error(`Failed to read QB Purchase Order: ${poResp.status} - ${await poResp.text()}`);
    const poData = await poResp.json();
    steps.push(`PO verified: #${poData.PurchaseOrder?.DocNumber}`);

    // Build line items at COST prices (mirrors qb-push-po)
    const qbLines: any[] = [];
    const selectedOptions = order.selected_options || [];
    const baseModel = order.base_models;

    if (baseModel) {
      qbLines.push({ DetailType: "ItemBasedExpenseLineDetail", Amount: baseModel.cost_price || 0, Description: baseModel.name || "", ItemBasedExpenseLineDetail: { ItemRef: { ...(baseModel.qb_item_id ? { value: baseModel.qb_item_id } : {}), name: baseModel.qb_item_name || FALLBACK_ITEM }, UnitPrice: baseModel.cost_price || 0, Qty: 1 } });
    ...
    if (Array.isArray(selectedOptions) && selectedOptions.length > 0) {
      ...
      for (const opt of selectedOptions) {
        ...
        qbLines.push({ DetailType: "ItemBasedExpenseLineDetail", Amount: amount, Description: desc, ItemBasedExpenseLineDetail: { ItemRef: { ...(mapping?.qb_item_id ? { value: mapping.qb_item_id } : {}), name: itemName }, UnitPrice: costEach, Qty: qty } });
      }
      steps.push(`options: ${selectedOptions.length} line items`);
    }

    if (order.freight_estimate && order.freight_estimate > 0) {
      qbLines.push({ DetailType: "ItemBasedExpenseLineDetail", Amount: order.freight_estimate, Description: "Freight", ItemBasedExpenseLineDetail: { ItemRef: { name: "Shipping Charges" }, UnitPrice: order.freight_estimate, Qty: 1 } });
      steps.push(`freight: $${order.freight_estimate}`);
    }

    if (qbLines.length === 0) {
      qbLines.push({ DetailType: "ItemBasedExpenseLineDetail", Amount: order.our_cost || 0, Description: order.build_shorthand || "Equipment", ItemBasedExpenseLineDetail: { ItemRef: { name: FALLBACK_ITEM }, UnitPrice: order.our_cost || 0, Qty: 1 } });
    ...
    const qbBill: any = {
      ...
      PrivateNote: `${orderLabel}${order.build_shorthand ? " - " + order.build_shorthand : ""}`.substring(0, 4000),
    };

    steps.push(`creating Bill: ${qbLines.length} lines linked to PO ${order.qb_po_id}`);
    const billResp = await fetch(`${baseUrl}/v3/company/${realm_id}/bill?minorversion=73`, { method: "POST", headers: { Authorization: `Bearer ${access_token}`, "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify(qbBill) });
    const billBody = await billResp.text();
    if (!billResp.ok) throw new Error(`QB ${billResp.status}: ${billBody}`);

    const billData = JSON.parse(billBody);
    const billId = billData.Bill.Id;
    const billDoc = billData.Bill.DocNumber;
    steps.push(`Bill created: #${billDoc}`);
