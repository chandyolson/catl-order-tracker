...
Deno.serve(async (req) => {
  ...
  try {
    ...
    if (!order.qb_estimate_id) throw new Error("No QB Estimate found on this order. Push an Estimate to QuickBooks first.");
    ...
    if (!customer?.qb_customer_id) throw new Error(`Customer "${customer?.name || "unknown"}" has no QuickBooks customer ID. Sync customers in Settings before converting to an Invoice.`);

    // Fetch the current estimate for its QB estimate ID and doc number
    const { data: estimate } = await supabase.from("estimates").select("*").eq("order_id", order_id).eq("is_current", true).maybeSingle();
    const qbEstimateId = order.qb_estimate_id;
    const qbEstimateDocNumber = estimate?.qb_doc_number || order.qb_estimate_doc_number || "";
    steps.push(`estimate: QB ID ${qbEstimateId}, doc #${qbEstimateDocNumber}`);
    ...
    // Build line items at RETAIL prices — same strategy as qb-push-estimate
    const qbLines: any[] = [];
    let lineNum = 1;
    ...
    if (Array.isArray(selectedOptions) && selectedOptions.length > 0) {
      ...
      for (const opt of selectedOptions) {
        ...
        qbLines.push({ Id: String(lineNum++), DetailType: "SalesItemLineDetail", Amount: amount, Description: desc, SalesItemLineDetail: { UnitPrice: retailEach, Qty: qty, ItemRef: { ...(itemId ? { value: itemId } : {}), name: itemName || "Services" } } });
      }
      steps.push(`options: ${selectedOptions.length} line items`);
    }

    if (order.freight_estimate && order.freight_estimate > 0) {
      qbLines.push({ Id: String(lineNum++), DetailType: "SalesItemLineDetail", Amount: order.freight_estimate, Description: "Freight", SalesItemLineDetail: { UnitPrice: order.freight_estimate, Qty: 1, ItemRef: { name: "Shipping Charges" } } });
    ...
    if (qbLines.length === 0) {
      const price = order.customer_price ?? 0;
    ...
    const qbInvoice: any = {
      ...
      LinkedTxn: [{ TxnId: qbEstimateId, TxnType: "Estimate" }],
      Line: qbLines,
    ...
    steps.push(`Invoice payload: ${qbLines.length} lines, linked to Estimate ${qbEstimateId}`);

    const qbResp = await fetch(`${baseUrl}/v3/company/${realm_id}/invoice?minorversion=75`, { method: "POST", headers: { Authorization: `Bearer ${access_token}`, "Content-Type": "application/json", Accept: "application/json" }, body: JSON.stringify(qbInvoice) });
    const qbBody = await qbResp.text();
    if (!qbResp.ok) throw new Error(`QB ${qbResp.status}: ${qbBody}`);

    const qbData = JSON.parse(qbBody);
    const invoiceId = qbData.Invoice.Id;
    const invoiceDoc = qbData.Invoice.DocNumber;
    steps.push(`Invoice created: #${invoiceDoc} (ID: ${invoiceId})`);

    await supabase.from("orders").update({ qb_invoice_id: invoiceId, qb_invoice_doc_number: invoiceDoc }).eq("id", order_id);
    await supabase.from("order_timeline").insert({ order_id: order.id, event_type: "invoice_created", title: "Customer invoice created in QuickBooks", description: `QB Invoice #${invoiceDoc} (ID: ${invoiceId}) linked to Estimate #${qbEstimateDocNumber}`, created_by: "system" });
