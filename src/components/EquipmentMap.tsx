...
const svgDot = (fill:string) => `data:image/svg+xml,${encodeURIComponent(`<svg xmlns="http://www.w3.org/2000/svg" width="20" height="20"><circle cx="10" cy="10" r="8" fill="${fill}" stroke="#fff" stroke-width="2"/></svg>`)}`;
...
export default function EquipmentMap() {
  ...
  const { data:orders=[], isLoading } = useQuery({
    ...
    queryFn: async()=>{
      const {data,error} = await supabase.from("orders")
        ...
        .not("customer_id","is",null).order("moly_contract_number",{ascending:false});
  ...
  const byState = useMemo(()=>{
    const m:Record<string,OrderPin[]>={}; orders.forEach(o=>{if(!m[o.state])m[o.state]=[];m[o.state].push(o);});
  ...
  useEffect(()=>{
    if(!mapLoaded||needsGeo.length===0) return;
    setGeocoding(true);
    const geocoder = new google.maps.Geocoder();
    const seen = new Set<string>();
    (async()=>{
      for(const o of needsGeo){
        if(seen.has(o.customerId)) continue; seen.add(o.customerId);
        try {
  ...
  useEffect(()=>{
    ...
    markersRef.current.push(hq); bounds.extend(CATL_HQ);

    const pinned=orders.filter(o=>o.lat&&o.lng);
    pinned.forEach(o=>{
      const pos={lat:o.lat!,lng:o.lng!}; bounds.extend(pos);
      const sc=SP[o.status]||SP.order_pending;
