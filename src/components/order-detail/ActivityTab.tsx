...
import { Plus, Phone, Mail, ExternalLink, ChevronDown } from "lucide-react";
...
export default function ActivityTab({ orderId, docs, events, changes, order, queryClient }: ActivityTabProps) {
  ...
  /* Timeline items (left): completed paperwork + all events + change orders, chronological */
  const timelineItems = useMemo(() => {
    ...
    for (const ev of events) {
      items.push({ id: `tl-${ev.id}`, type: "timeline", date: ev.created_at ? new Date(ev.created_at) : new Date(), data: ev });
    }
    for (const co of changes) {
      items.push({ id: `co-${co.id}`, type: "change_order", date: co.created_at ? new Date(co.created_at) : new Date(), data: co });
    }
    items.sort((a, b) => a.date.getTime() - b.date.getTime());
    return items;
  }, [completeDocs, events, changes]);

  /* To-do items (right): pending + blocked paperwork */
  const todoItems = useMemo(() => {
    return [
      ...pendingDocs.map((d) => ({ ...d, _blocked: false })),
      ...blockedDocs.map((d) => ({ ...d, _blocked: true })),
    ];
  }, [pendingDocs, blockedDocs]);

  /* Mutations */
  const markCompleteMutation = useMutation({
    mutationFn: async ({ docId, docType }: { docId: string; docType: string }) => {
      const today = format(new Date(), "yyyy-MM-dd");
      const { error } = await supabase.from("paperwork").update({ status: "complete", completed_date: today, updated_at: new Date().toISOString() }).eq("id", docId);
      if (error) throw error;
      await supabase.from("order_timeline").insert({ order_id: orderId, event_type: "document_signed", title: `${DOC_NAMES[docType] || docType} completed` });
    },
    onSuccess: () => {
  ...
  const addEntryMutation = useMutation({
    mutationFn: async () => {
      const { error } = await supabase.from("order_timeline").insert({
        order_id: orderId, event_type: entryType,
        title: entryTitle || (entryType === "phone_call" ? "Phone call" : entryType === "email" ? "Email" : "Note"),
        description: entryDesc || null, contact_method: entryType === "note" ? null : entryType, contact_with: contactWith || null,
      });
      if (error) throw error;
    },
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ["order_timeline", orderId] });
      setShowForm(false); setEntryTitle(""); setEntryDesc(""); setContactWith("");
  ...
  const toggleChangeCascade = useMutation({
    ...
    onSuccess: () => { queryClient.invalidateQueries({ queryKey: ["change_orders", orderId] }); },
  });

  return (
    <div>
      <div className="flex items-center justify-between mb-3">
        <span className="text-[11px] font-medium text-muted-foreground">{completeCount}/{totalCount} paperwork complete</span>
      </div>

      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        {/* LEFT: Timeline */}
        <div>
          <p className="text-[11px] font-medium tracking-wider uppercase mb-2" style={{ color: "#717182" }}>Timeline</p>
          {timelineItems.length === 0 ? (
            <p className="text-[12px] text-muted-foreground py-2">No activity yet.</p>
          ) : (
            <div className="relative pl-5">
              <div className="absolute left-[3px] top-2 bottom-2 w-px" style={{ backgroundColor: "#E8E8E3" }} />
              {timelineItems.map((item) => (
                <div key={item.id}>
                  {item.type === "paperwork" && <CompletedPaperworkRow doc={item.data} />}
                  {item.type === "timeline" && <TimelineEventRow event={item.data} />}
                  {item.type === "change_order" && <ChangeOrderRow co={item.data} onToggleCascade={(coId, field, value) => toggleChangeCascade.mutate({ coId, field, value })} />}
                </div>
              ))}
            </div>
          )}
          ...
          <div className="mt-3">
            {!showForm && (
              <div className="flex gap-2">
                <button onClick={() => { setEntryType("note"); setShowForm(true); }} className="flex items-center gap-1.5 text-[12px] font-semibold rounded-full px-3 py-1.5 active:scale-[0.97] transition-transform" style={{ border: "1px solid #55BAAA", color: "#55BAAA" }}><Plus size={12} /> Note</button>
                <button onClick={() => { setEntryType("phone_call"); setShowForm(true); }} className="flex items-center gap-1.5 text-[12px] font-semibold rounded-full px-3 py-1.5 active:scale-[0.97] transition-transform" style={{ border: "1px solid #5B8DEF", color: "#5B8DEF" }}><Phone size={12} /> Call</button>
                <button onClick={() => { setEntryType("email"); setShowForm(true); }} className="flex items-center gap-1.5 text-[12px] font-semibold rounded-full px-3 py-1.5 active:scale-[0.97] transition-transform" style={{ border: "1px solid #B8860B", color: "#B8860B" }}><Mail size={12} /> Email</button>
              </div>
            )}
          </div>
          ...
          {showForm && (
            <div className="bg-card border border-border rounded-lg p-3 mt-2 space-y-2">
              <span className="text-[12px] font-semibold text-foreground">{entryType === "phone_call" ? "Phone call" : entryType === "email" ? "Email" : "Note"}</span>
              {(entryType === "phone_call" || entryType === "email") && (
                <input value={contactWith} onChange={(e) => setContactWith(e.target.value)} placeholder="Who did you talk to?" className="w-full border border-border rounded-lg px-3 py-2 bg-card text-sm outline-none text-[16px]" />
              )}
              <input value={entryTitle} onChange={(e) => setEntryTitle(e.target.value)} placeholder={entryType === "phone_call" ? "What was discussed?" : entryType === "email" ? "Email subject" : "Note title"} className="w-full border border-border rounded-lg px-3 py-2 bg-card text-sm outline-none text-[16px]" />
              <textarea value={entryDesc} onChange={(e) => setEntryDesc(e.target.value)} placeholder="Details (optional)" rows={3} className="w-full border border-border rounded-lg px-3 py-2 bg-card text-sm outline-none resize-none text-[16px]" />
              <div className="flex gap-2">
                <button onClick={() => addEntryMutation.mutate()} disabled={addEntryMutation.isPending || !entryTitle.trim()} className="px-4 py-2 rounded-full text-sm font-bold active:scale-[0.97] disabled:opacity-50" style={{ backgroundColor: "#F3D12A", color: "#0E2646" }}>Save</button>
                <button onClick={() => { setShowForm(false); setEntryTitle(""); setEntryDesc(""); setContactWith(""); }} className="text-sm text-muted-foreground px-3">Cancel</button>
              </div>
            </div>
          )}
        </div>
        ...
        <div>
          <div className="flex items-center justify-between mb-2">
            <p className="text-[11px] font-medium tracking-wider uppercase" style={{ color: "#717182" }}>To do</p>
            <span className="text-[11px] text-muted-foreground">{pendingDocs.length} remaining</span>
          </div>
          {todoItems.length === 0 ? (
            <div className="text-center py-6">
              <p className="text-[13px] font-medium text-foreground">All done!</p>
              <p className="text-[11px] text-muted-foreground mt-1">Every paperwork item is complete.</p>
            </div>
          ) : (
            <div className="flex flex-col gap-1">
              {todoItems.map((doc) => {
                const isBlocked = doc._blocked;
                ...
                return (
                  <div key={doc.id} className={cn("flex items-center gap-2 py-2 px-3 rounded-lg bg-card border border-border transition-colors", isBlocked ? "opacity-40" : "cursor-pointer hover:bg-muted/50")} onClick={() => { if (!isBlocked && !markCompleteMutation.isPending) markCompleteMutation.mutate({ docId: doc.id, docType: doc.document_type }); }}>
                    <div className="w-4 h-4 rounded shrink-0" style={{ border: isBlocked ? "1.5px solid #B4B2A9" : "1.5px solid #F3D12A" }} />
                    <span className={cn("text-[12px] flex-1 min-w-0", isBlocked ? "text-muted-foreground" : "font-medium text-foreground")}>{name}</span>
                    <span className="text-[10px] px-1.5 py-0.5 rounded-full shrink-0" style={{ background: trackStyle.bg, color: trackStyle.color }}>{track}</span>
                    {isBlocked && doc.blocked_reason && <span className="text-[10px] text-muted-foreground shrink-0 truncate max-w-[100px]">{doc.blocked_reason}</span>}
                  </div>
                );
              })}
            </div>
...
/* Completed Paperwork in Timeline */
function CompletedPaperworkRow({ doc }: { doc: any }) {
  const track = DOC_TRACK[doc.document_type] || "Customer";
  const trackStyle = TRACK_STYLE[track] || TRACK_STYLE.Customer;
  const name = DOC_NAMES[doc.document_type] || doc.document_type;
  ...
  return (
    <div className="relative pb-2 pl-6">
      <div className="absolute left-0 top-[8px] w-[7px] h-[7px] rounded-full" style={{ backgroundColor: "#27AE60", border: "2px solid #F5F5F0" }} />
      <div className="flex items-center gap-2 py-1.5 px-3 rounded-lg bg-card border border-border">
        ...
        <span className="text-[10px] px-1.5 py-0.5 rounded-full shrink-0" style={{ background: trackStyle.bg, color: trackStyle.color }}>{track}</span>
        {doc.file_url && (
          <a href={doc.file_url} target="_blank" rel="noopener noreferrer" onClick={(e) => e.stopPropagation()} className="shrink-0" title="View document">
            <ExternalLink size={12} style={{ color: "#55BAAA" }} />
          </a>
        )}
...
function TimelineEventRow({ event }: { event: any }) {
  ...
  if (isConversation) {
    return (
      <div className="relative pb-2 pl-6">
        <div className="absolute left-0 top-[8px] w-[7px] h-[7px] rounded-full" style={{ backgroundColor: dotColor, border: "2px solid #F5F5F0" }} />
        <div className="bg-card border border-border rounded-lg p-2.5">
          <div className="flex justify-between items-center mb-1">
            <div className="flex items-center gap-1.5">
              {event.event_type === "phone_call" ? <Phone size={12} style={{ color: "#5B8DEF" }} /> : <Mail size={12} style={{ color: "#B8860B" }} />}
          ...
          {event.title && event.contact_with && <p className="text-[12px] font-medium text-foreground mb-0.5">{event.title}</p>}
          {event.description && (
            <p className={cn("text-[11px] text-muted-foreground", !expanded && "line-clamp-2 cursor-pointer")} onClick={() => setExpanded(!expanded)}>{event.description}</p>
          )}
        </div>
      </div>
    );
  ...
  return (
    <div className="relative pb-2 pl-6">
      <div className="absolute left-0 top-[7px] w-[7px] h-[7px] rounded-full" style={{ backgroundColor: dotColor, border: "2px solid #F5F5F0" }} />
      {hasDescription ? (
        <div className="bg-card border border-border rounded-lg p-2.5">
...
function ChangeOrderRow({ co, onToggleCascade }: { co: any; onToggleCascade: (coId: string, field: string, value: boolean) => void }) {
  ...
  return (
    <div className="relative pb-2 pl-6">
      <div className="absolute left-0 top-[8px] w-[7px] h-[7px] rounded-full" style={{ backgroundColor: "#D4183D", border: "2px solid #F5F5F0" }} />
      <div className="bg-card border border-border rounded-lg p-2.5" style={{ borderColor: allApplied ? "#27AE60" : "#E8863A" }}>
        ...
        <div className="flex gap-1.5 flex-wrap mb-1.5">
          {co.source && (
            <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium" style={{ background: co.source === "customer" ? "#E1F5EE" : co.source === "moly" ? "#FAEEDA" : "#F1EFE8", color: co.source === "customer" ? "#085041" : co.source === "moly" ? "#633806" : "#5F5E5A" }}>
              {co.source === "customer" ? "Customer" : co.source === "moly" ? "MOLY" : "Internal"}
            </span>
          )}
          {allApplied && <span className="text-[10px] px-1.5 py-0.5 rounded-full font-medium" style={{ background: "#E1F5EE", color: "#085041" }}>All applied</span>}
        </div>
        ...
        {Array.isArray(co.changes_summary) && co.changes_summary.length > 0 && (
          <div className="space-y-0.5 mb-1.5">
            {(co.changes_summary as any[]).map((change: any, i: number) => (
              <div key={i} className="text-[11px]" style={{ color: change.type === "added" ? "#27AE60" : change.type === "removed" ? "#D4183D" : "#B8860B" }}>
                {change.type === "added" ? "+ " : change.type === "removed" ? "− " : "~ "}{change.option || change.field}{change.detail ? ` (${change.detail})` : ""}{change.from && change.to ? `: ${change.from} → ${change.to}` : ""}
              </div>
            ))}
          </div>
        )}
