...
const PF: Record<string,string> = {start:"#0E2646",end:"#0E2646",pickup:"#55BAAA",delivery:"#E24B4A",waypoint:"#888780",catl:"#F3D12A"};
const PL: Record<string,string> = {start:"S",end:"E",pickup:"P",delivery:"D",waypoint:"W",catl:"C"};
...
function loadMapsApi(): Promise<void> {
  return new Promise((resolve, reject) => {
    ...
    s.src = `https://maps.googleapis.com/maps/api/js?key=${GKEY}&libraries=places,geometry&callback=__gmcb__`;
...
export default function FreightMap({ points, totalMiles, onMilesCalculated, onSaveMiles, height = 280 }: FreightMapProps) {
  ...
  // Create a detached div for the map so React never touches it
  useEffect(() => {
    ...
    loadMapsApi().then(() => {
      mapRef.current = new google.maps.Map(div, {
  ...
  const calculateRoute = useCallback(async () => {
    ...
    try {
      ...
      const geo: { pt: RoutePoint; loc: google.maps.LatLng }[] = [];
      for (const pt of points) {
        const a = pt.address ? `${pt.address}, ${pt.city}, ${pt.state}` : `${pt.city}, ${pt.state}`;
        try { const r = await gc.geocode({ address: a }); if (r.results[0]) geo.push({ pt, loc: r.results[0].geometry.location }); } catch (e) { console.warn(`Geocode fail ${a}`); }
      }
      if (geo.length < 2) { setError("Could not geocode enough addresses."); setLoading(false); return; }
      markersRef.current.forEach(m => m.setMap(null)); markersRef.current = [];
      const bounds = new google.maps.LatLngBounds();
      geo.forEach(({ pt, loc }) => {
        bounds.extend(loc);
        const marker = new google.maps.Marker({
          map: mapRef.current!, position: loc,
          icon: { url: svgPin(PF[pt.type] || "#888", PL[pt.type] || "•", pt.type === "start" || pt.type === "end" ? "#F3D12A" : "#fff"), scaledSize: new google.maps.Size(28, 28), anchor: new google.maps.Point(14, 14) },
          title: `${pt.label}${pt.subtitle ? " — " + pt.subtitle : ""}`,
        });
        const iw = new google.maps.InfoWindow({ content: `<div style="font-family:Inter,system-ui,sans-serif;padding:4px 0;"><strong style="font-size:13px;color:#0E2646;">${pt.label}</strong>${pt.subtitle ? `<br><span style="font-size:11px;color:#717182;">${pt.subtitle}</span>` : ""}<br><span style="font-size:11px;color:#717182;">${pt.city}, ${pt.state}</span></div>` });
        marker.addListener("click", () => iw.open({ anchor: marker, map: mapRef.current! }));
        markersRef.current.push(marker);
      });
      mapRef.current.fitBounds(bounds, { top: 40, bottom: 40, left: 40, right: 40 });
      const wps = geo.slice(1, -1).map(g => ({ location: g.loc, stopover: true }));
      const dr = await ds.route({ origin: geo[0].loc, destination: geo[geo.length - 1].loc, waypoints: wps, travelMode: google.maps.TravelMode.DRIVING, optimizeWaypoints: false });
      if (dr.routes[0]) {
        rendererRef.current?.setDirections(dr);
  ...
  return (
    <div>
      <div className="rounded-xl overflow-hidden mb-3" style={{ border: "0.5px solid #D4D4D0" }}>
        ...
        <div ref={containerRef} style={{ minHeight: height, backgroundColor: "#E8E6DE" }}>
          {!ready && !error && <div className="flex items-center justify-center" style={{ height }}><p className="text-[13px]" style={{ color: "#717182" }}>Loading map...</p></div>}
        </div>
      </div>
      {(legs.length > 0 || loading) && (
        <div className="rounded-xl p-3" style={{ backgroundColor: "#fff", border: "0.5px solid #D4D4D0" }}>
          ...
          <div className="space-y-0.5 mb-3">
            ...
            {legs.length > 0 && <div className="flex items-center gap-2"><div className="flex flex-col items-center" style={{ width: 12 }}><div className="w-2 h-2 rounded-full" style={{ backgroundColor: "#0E2646" }} /></div><span className="text-[12px]" style={{ color: "#1A1A1A" }}>{legs[legs.length - 1].to}</span></div>}
          </div>
