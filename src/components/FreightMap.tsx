...
async function loadGoogleMaps() {
  ...
  if (document.querySelector('script[src*="maps.googleapis.com"]')) { mapsLoaded = true; return; }
  return new Promise<void>((resolve, reject) => {
    const s = document.createElement("script");
    s.src = `https://maps.googleapis.com/maps/api/js?key=${GKEY}&libraries=places,geometry`;
  ...
  useEffect(() => {
    ...
    loadGoogleMaps().then(() => {
      ...
      mapInstance.current = new google.maps.Map(mapRef.current, {
        center:{lat:42.5,lng:-100}, zoom:6, mapTypeControl:false, streetViewControl:false, fullscreenControl:false,
  ...
  const calculateRoute = useCallback(async () => {
    ...
    try {
      ...
      geocoded.forEach(({point,location})=>{
        ...
        const fill=PIN_FILL[point.type]||"#888", label=PIN_LABEL[point.type]||"•";
        ...
        const info = new google.maps.InfoWindow({content:`<div style="font-family:Inter,system-ui,sans-serif;padding:4px 0;"><strong style="font-size:13px;color:#0E2646;">${point.label}</strong>${point.subtitle?`<br><span style="font-size:11px;color:#717182;">${point.subtitle}</span>`:""}<br><span style="font-size:11px;color:#717182;">${point.city}, ${point.state}</span></div>`});
      ...
      const origin=geocoded[0].location, destination=geocoded[geocoded.length-1].location;
      ...
      if (dirResult.routes[0]) {
        ...
        const legResults: LegResult[] = dirResult.routes[0].legs.map((leg,i)=>({
          from:geocoded[i].point.label, to:geocoded[i+1]?.point.label||geocoded[geocoded.length-1].point.label,
          miles:Math.round((leg.distance?.value||0)*0.000621371), duration:leg.duration?.text||"",
        }));
